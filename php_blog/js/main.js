...
function CheckTime() {
    ...
    if (total_sekunden <= 0) {
        $(function () {
            ...
            $(dialogtext).text('Die Zeit ist vorbei! Versuchen Sie es noch einmal!');
...
$(document).ready(function () {
    ...
    }); // ENDE click button Dokumentation
    ...
    $('#avatarToUpload').on('change', function () { //on file input change
        if (window.File && window.FileReader && window.FileList && window.Blob) { //check File API supported browser

            $('#avatar-output').html(''); //clear html of output element
            var data = $(this)[0].files; //this file data
            ...
            $.each(data, function (index, file) { //loop though each file
                if (/(\.|\/)(gif|jpe?g|png)$/i.test(file.type)) { //check supported file type
                    ...
                    fRead.onload = (function (file) { //trigger function on successful read
                        return function (e) {
                            var img = $('<img/>').addClass('thumb').attr('src', e.target.result); //create image element 
                            $('#avatar-output').append(img); //append image to output element
                        };
                    })(file);
                    fRead.readAsDataURL(file); //URL representing the file's data.
                }
    ...
    }); // ENDE Avatar anzeigen

    $('#avatar2ToUpload').on('change', function () { //on file input change
        if (window.File && window.FileReader && window.FileList && window.Blob) { //check File API supported browser

            $('#avatar2-output').html(''); //clear html of output element
            var data = $(this)[0].files; //this file data
            ...
            $.each(data, function (index, file) { //loop though each file
                if (/(\.|\/)(gif|jpe?g|png)$/i.test(file.type)) { //check supported file type
                    ...
                    fRead.onload = (function (file) { //trigger function on successful read
                        return function (e) {
                            var img = $('<img/>').addClass('thumb').attr('src', e.target.result); //create image element 
                            $('#avatar2-output').append(img); //append image to output element
                        };
                    })(file);
                    fRead.readAsDataURL(file); //URL representing the file's data.
                }
    ...
    }); // ENDE Der neue Avatar anzeigen


    /* $("form").submit(function () {
     $('#table_comments').html(''); //clear html of output element
     //$('#tabelle_comments').html(showComments($link, $post_id, $id)); 
     window.location.href = 'artikel.php?id=15'; 
     });*/
